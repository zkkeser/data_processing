...
def scrape_movie_page(dom):
    ...
    for a in dom.by_tag("time")[:1]:
        duration = plaintext(a.content).encode("utf-8")
        print duration
    ...
    for a in dom.by_tag("div.infobar")[:1]:
        ...
        for b in a.by_tag("span.itemprop"):
            genre = plaintext(b.content).encode("utf-8")
            genre_list.append(genre)
        print genre_list
    #directors
    for a in dom.by_tag("div.txt-block")[:1]:
        for b in a.by_tag("a"):
            directors = plaintext(b.content).encode("utf-8")
            print directors
...
'''
    ...
    #writers
    for a in dom.by_tag("span.itemprop")[:1]:
        writers = plaintext(a.content).encode("utf-8")
        print writers
    #actors
    for a in dom.by_tag("span.itemprop")[:1]:
        actors = plaintext(a.content).encode("utf-8")
        print actors
    #rating
    for a in dom.by_tag("span.itemprop")[:1]:
        rating = plaintext(a.content).encode("utf-8")
        print rating
    #n_ratings
    for a in dom.by_tag("span.itemprop")[:1]:
        n_ratings = plaintext(a.content).encode("utf-8")
        print n_ratings
'''
