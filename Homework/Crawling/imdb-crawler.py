...
def scrape_movie_page(dom):
    ...
    for a in dom.by_tag("span.itemprop")[:1]:
    ...
    for a in dom.by_tag("time")[:1]:
        duration = plaintext(a.content)[:-4]
        print duration
    print

    #genres
    
    for a in dom.by_tag("div.infobar")[:1]:
        genres_unjoined = []
        for b in a.by_tag("span.itemprop"):
            genre = plaintext(b.content)
            genres_unjoined.append(genre)
        genres = ";".join(genres_unjoined)
        print genres
    print

    #directors
    
    for a in dom.by_tag("div.txt-block")[:1]:
        directors_unjoined = []
        for b in a.by_tag("a"):
            director = plaintext(b.content)
            directors_unjoined.append(director)
        directors = ";".join(directors_unjoined)
        print directors
    print
    ...
    for a in dom.by_tag("div.txt-block")[1:2]:
        writers_unjoined = []
        for b in a.by_tag("span.itemprop"):
            writer = plaintext(b.content)
            writers_unjoined.append(writer)
        writers = ";".join(writers_unjoined)
        print writers
    print
    ...
    for a in dom.by_tag("div.txt-block")[2:3]:
        actors_unjoined = []
        for b in a.by_tag("span.itemprop")[:3]:
            actor = plaintext(b.content)
            actors_unjoined.append(actor)
        actors = ";".join(actors_unjoined)    
        print actors
    print
    
    #ratings
            
    for a in dom.by_tag("div.titlePageSprite star-box-giga-star"):
        rating = plaintext(a.content)
        print rating
    print
    ...
    for a in dom.by_tag("div.star-box-details"):
        for b in a.by_tag("a")[:1]:
            old_n_ratings = plaintext(b.content)[:-6]
            n_ratings = old_n_ratings.replace(",","")
            print n_ratings
    print
